    ...
    if ( isset( $truck ) ){
        ...
        $card = ( object ) [
            ...
            'location'  => ( object ) [
                ...
                'state'             => $truck->STATE,
                'zip'               => null,
                'city_state'        => ( isset( $truck->CITY_STATE ) ) ? $truck->CITY_STATE : "{$truck->LOCATION}, {$truck->STATE}"
            ],
            'truck'     => ( object ) [
...
<a href="<?php echo $card->href; ?>"
    class="FEATURED-TRUCK-CARD LOCATION-ID LOCATION-STATE relative block overflow-hidden truck-card bg-gray-100"
  ...
  <div class="card-details bg-gray-100">
    ...
    <div class="card-meta text-gray-300 <?php echo $defaults[ 'all' ] ? 'text-xs md:text-base' : 'text-xs'; ?>">
        <?php
            ...
            if($truck->LOCATION == 'Chicago'){
                $daLocation = 'Chicago, IL';
            }
            elseif($truck->LOCATION == 'St. Louis'){
                $daLocation = 'St. Louis, MO';
            }
            else{
                $daLocation = $truck->LOCATION . ', ' . $truck->STATE;
            }

        ?>
      <p><svg class="icon icon-pin svg-align text-lg mr-1"><use xlink:href="#icon-pin"></use></svg> <?php echo $daLocation ?></p>
    </div> <!-- /.flex -->
